...
/**
 * @notice Deployment, role lifecycle and transient-owner cases for `KSAllowanceHubV2`
 * @dev Covers LIF-01..LIF-12 of the frozen plan. Only the wiring this hub performs on the inherited
 * `ks-common-sc` management bases is asserted here; the bases' own mechanics are covered upstream.
 */
contract KSAllowanceHubV2LifecycleTest is KSAllowanceHubV2Base {
  ...
  /// @notice LIF-01 the constructor wires Permit2, the admin, guardians, rescuers and the routers
  function test_constructorWiring() public {
    ...
    KSAllowanceHubV2 seeded =
      new KSAllowanceHubV2(admin, guardians, rescuers, routers, address(permit2));
    KSAllowanceHubV2 empty =
      new KSAllowanceHubV2(admin, guardians, rescuers, new address[](0), address(permit2));
  ...
  /// @notice LIF-02 the constructor sets `GUARDIAN_ROLE` as the revoker of `WHITELIST_ROUTER_ROLE`
  ...
  /// @notice LIF-03 a guardian can drop a router, after which calling it is rejected
  ...
  /// @notice LIF-04 guardian and admin can both pause, but only the admin can unpause
  ...
  /// @notice LIF-06 the transient owner reads as zero outside a call and is cleared afterwards
  ...
  /// @notice LIF-07 the lock is released at the end of a call, so a second call succeeds
  ...
  /// @notice LIF-08 unspent `msg.value` is stranded in the hub and only recoverable by a rescuer
  ...
  /// @notice LIF-09 the admin can whitelist a new router, which then executes successfully
  ...
  /// @notice LIF-10 nobody but a guardian or the admin can pause the hub
  ...
  /// @notice LIF-11 the admin can revoke `WHITELIST_ROUTER_ROLE`
  ...
  /// @notice LIF-12 an account that is neither a guardian nor the admin cannot revoke the role
