...
/**
 * @notice `KSAllowanceHub.permitTransferAndExecute` — the legacy hub
 * @dev Tokens on this entrypoint are always pulled from `msg.sender`, so `owner` is both the signer
 * and the caller throughout. The legacy `TransferTokens` event carries no `nativeTransfers` member,
 * and the legacy hub declares neither `receive()` nor `fallback()`, so native balances it is meant
 * to already hold are staged with `vm.deal`.
 */
contract KSAllowanceHubPermitTransferAndExecuteTest is KSAllowanceHubBase {
  using ArrayHelper for *;

  /// @dev Fuzz inputs for the conservation property; one struct, every field bounded before use
  ...
  /// @dev `Transfer(address,address,uint256)`, the ERC20 event a zero-amount entry never emits
  bytes32 internal constant ERC20_TRANSFER_TOPIC = keccak256('Transfer(address,address,uint256)');

  /* ------------------------------------------------------------ ERC20 fan-out */

  /// @notice One ERC20, one target, pre-existing allowance
  ...
  /// @notice One ERC20 fanned out to three targets with distinct amounts
  ...
  /// @notice Two `ERC20Params` entries flatten param-major then target-major
  ...
  /// @notice Both native sentinels are honoured, in one execution, out of `msg.value`
  function test_bothNativeSentinels_payOutOfMsgValue() public {
    ...
    assertEq(owner.balance, 0, 'the caller paid the whole msg.value');
  }

  /* -------------------------------------------------------------- ERC20 permit */

  /// @notice A valid 160-byte EIP-2612 payload establishes the allowance on the fly
  ...
  /// @notice Empty and malformed ERC20 permitData both fall through to the prior approve
  ...
  /// @notice A well-shaped but wrongly signed permit is swallowed, then the transfer fails
  function test_badPermitSignature_isSwallowedThenTransferFails() public {
    ...
    vm.prank(owner);
    // The permit revert never surfaces: what bubbles is the later `transferFrom` failure, wrapped
    // in the ERC-7751 envelope `CustomRevert.bubbleUpAndRevertWith` builds
    vm.expectRevert(
    ...
    assertEq(tokenA.nonces(owner), 0, 'the rejected permit consumed no nonce');
  }

  /* ------------------------------------------------------------- ERC721 permit */

  /// @notice A 128-byte v3 ERC721 permit approves the hub and the token moves
  ...
  /// @notice A 224-byte v4 ERC721 permit takes the 7-word branch
  ...
  /// @notice A swallowed permit and an absent permit both fall back on `setApprovalForAll`
  ...
  /// @notice `targets.length != amounts.length` is rejected
  ...
  /**
   * @notice The exact legacy `TransferTokens` payload
   * @dev The legacy event has five members: value-bearing generic calls are not reported anywhere,
   * unlike V2's sixth `nativeTransfers` member.
   */
  function test_transferTokensPayload_isExact() public {
    ...
    emit TransferTokens(owner, owner, 3 ether, expected20, expected721);

    vm.prank(owner);
    hub.permitTransferAndExecute{value: 3 ether}(erc20Params, erc721Params, genericCalls);

    assertEq(address(routerB).balance, 1 ether, 'the second call was funded');
    assertEq(address(routerA).balance, 2 ether, 'the third call was funded');
  }

  /// @notice `results` mirrors each router's configured return data, in call order
  ...
  /// @notice The router reads the token owner back through `msgSender()`
  function test_routerObservesMsgSender() public {
    ...
    assertEq(hub.msgSender(), address(0), 'the slot is released once the call returns');
  }

  /// @notice `gasUsed` is non-zero and bounded by the gas available at the call site
  ...
  /// @notice A paused hub rejects the entrypoint
  ...
  /// @notice Spending more native token than `msg.value` is rejected
  function test_genericCallValueAboveMsgValue_reverts() public {
    // The legacy hub has no `receive()`, so the pre-existing balance is staged directly
  ...
  /// @notice Spending exactly `msg.value` is the accepted boundary
  ...
  /// @notice A router without `WHITELIST_ROUTER_ROLE` cannot be called
  ...
  /// @notice A reverting router bubbles its own error up unchanged
  function test_routerRevert_bubblesUp() public {
    ...
    assertEq(routerB.callCount(), 0, 'the earlier call was rolled back too');
  }

  /// @notice A router reentering the entrypoint hits the transient lock
  ...
  /// @notice The lock already holds at the ERC721 receiver hook, before any generic call
  function test_reentryFromErc721Receiver_isLocked() public {
    ...
    assertEq(
      ...
      abi.encodePacked(IKSAllowanceHub.AlreadyLocked.selector),
  ...
  /// @notice A native target that rejects value fails the whole call
  function test_nativeTargetRejects_reverts() public {
    ...
    assertEq(address(hub).balance, 0, 'the whole call was rolled back');
  }

  /* ------------------------------------------------------------- degenerate input */

  /// @notice An entirely empty call still succeeds and reports empty arrays
  function test_allArraysEmpty_succeeds() public {
    vm.expectEmit(true, true, false, true, address(hub));
    emit TransferTokens(owner, owner, 0, new ERC20Transfer[](0), new ERC721Transfer[](0));

    vm.prank(owner);
    (bytes[] memory results, uint256 gasUsed) =
      hub.permitTransferAndExecute(_noErc20Params(), _noErc721Params(), _noGenericCalls());

    assertEq(results.length, 0, 'no results');
    assertGt(gasUsed, 0, 'the empty body still consumed gas');
  }

  /// @notice A zero amount short-circuits in `TokenHelper` but is still reported
  ...
  /// @notice The transient owner slot is rolled back on every revert path
  function test_transientOwnerCleared_afterEveryRevert() public {
    ...
    assertEq(
      ret,
      abi.encodePacked(IKSAllowanceHub.NativeTokenOverspent.selector),
  ...
  /// @notice ERC20 conservation, and the hub stranding whatever `msg.value` is left over
  ...
  /// @dev Locates the hub's single legacy `TransferTokens` log and decodes its five members
    ...
    )
  {
    ...
    for (uint256 i = 0; i < logs.length; i++) {
      ...
      (msgValue, erc20Transfers, erc721Transfers) =
