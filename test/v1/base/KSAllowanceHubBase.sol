...
/**
 * @notice Shared fixture for every legacy KSAllowanceHub batch
 * @dev Mirrors the V2 fixture and reuses the same mocks, but deploys the legacy hub and declares
 * the legacy five-field `TransferTokens` event. Holds no test functions so Foundry does not
 * rediscover them per child.
 */
abstract contract KSAllowanceHubBase is Test {
  using ArrayHelper for *;

  /* ---------------------------------------------------------------- events */

  /// @dev The legacy event carries no `nativeTransfers` member, unlike V2's
  ...
  function setUp() public virtual {
    ...
    hub = new KSAllowanceHub(admin, guardians, rescuers, new address[](0), address(permit2));
